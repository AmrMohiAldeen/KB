...
- Item IDs are generated on insertion and retained through JSON/HTML serialization.
...
- Reordering replaces the container's child fragment in one transaction, so it is one undo step.
