...
const getNotificationIcon = ({ icon, color, skin }: Pick<NotificationsType, 'icon' | 'color' | 'skin'>) => {
  return (
    <CustomAvatar color={color} skin={skin || 'light-static'}>
      <i className={icon} />
    </CustomAvatar>
  )
}
...
const NotificationDropdown = ({ notifications }: { notifications: NotificationsType[] }) => {
  ...
  const handleReadNotification = (event: MouseEvent<HTMLElement>, value: boolean, index: number) => {
    ...
    // TODO: Replace with backend API call to PATCH /api/kb/notifications/{notificationId}/read.
    // Expected action: update readAt for the current SSO user's notification and return the updated unread count.
                ...
                <div>
                  ...
                  <ScrollWrapper hidden={hidden}>
                    {notificationsState.map((notification, index) => {
                      const { title, subtitle, time, read, icon, color, skin } = notification
                      ...
                      return (
                        ...
                        >
                          {getNotificationIcon({ icon, color, skin })}
