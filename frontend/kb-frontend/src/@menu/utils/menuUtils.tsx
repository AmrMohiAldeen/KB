...
type MenuChildProps = {
  label?: ReactNode
  component?: ReactElement<ComponentWithHref> | string
  href?: string
  children?: ReactNode
...
export const confirmUrlInChildren = (children: ChildrenType['children'], url: string): boolean => {
  ...
  if (isValidElement(children)) {
    const element = children as ReactElement<MenuChildProps>
    const { component, href, children: subChildren } = element.props

    if (component && typeof component !== 'string' && isValidElement(component)) {
      const componentElement = component as ReactElement<ComponentWithHref>

      if (componentElement.props.href === url) {
        return true
      }
    }
...
export const mapHorizontalToVerticalMenu = (children: ChildrenType['children']) => {
  return Children.map(children, child => {
    if (isValidElement(child)) {
      const element = child as ReactElement<MenuChildProps>
      const { children, verticalMenuProps, label, ...rest } = element.props
      ...
      switch (element.type) {
        ...
        case HorizontalSubMenu:
          return <VerticalSubMenu label={label} {...rest}>{mapHorizontalToVerticalMenu(children)}</VerticalSubMenu>

        case HorizontalMenu:
          return <VerticalMenu {...verticalMenuProps}>{mapHorizontalToVerticalMenu(children)}</VerticalMenu>
