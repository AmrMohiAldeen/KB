        ...
        >
          ...
          <div className='max-h-[55vh] overflow-y-auto p-3'>
            ...
            {!loading && !error && items.length === 0 && (
              <p className='p-4 text-sm text-gray-500'>No active media matched your search.</p>
            )}
...
export function MediaControls({
  ...
  return (
    <>
      <ToolbarDropdown
        title='Insert uploaded media'
        label={<><Paperclip className='h-4 w-4' /> Media</>}
        menuClassName='w-56'
      >
        <DropdownItem onActivate={() => open('image')}>
          <span className='flex items-center gap-2'><ImagePlus className='h-4 w-4' /> Upload image or GIF</span>
        </DropdownItem>
        ...
        <DropdownItem onActivate={() => open('library')}>
          <span className='flex items-center gap-2'><FolderOpen className='h-4 w-4' /> Select from library</span>
        </DropdownItem>
      </ToolbarDropdown>
