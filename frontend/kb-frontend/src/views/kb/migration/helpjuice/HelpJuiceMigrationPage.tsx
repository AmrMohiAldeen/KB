...
const HelpJuiceMigrationPage = ({ accessToken, api = helpJuiceMigrationsApi }: HelpJuiceMigrationPageProps) => {
  ...
  return <KbPageShell>
    ...
    <KbWorkflowDialog open={confirmOpen} title='Confirm HelpJuice migration' description={`The backend will validate and import ${preview?.totalArticles??0} articles in one request using ${options.conflictBehavior} conflict handling.`} notice='This synchronous operation can take a long time. Do not close the page; cancellation may leave already committed records in place.' confirmLabel='Start migration' onClose={()=>setConfirmOpen(false)} onConfirm={()=>void run()}><Stack spacing={1}><Typography>{options.importPublished?'Published articles included':'Published articles excluded'}</Typography><Typography>{options.importUnpublishedAsDrafts?'Unpublished articles imported as drafts':'Unpublished articles excluded'}</Typography><Typography>{options.importCategories?'Categories included':'Categories excluded'} · {options.importMedia?'Media included':'Media excluded'}</Typography></Stack></KbWorkflowDialog>
  </KbPageShell>
}
