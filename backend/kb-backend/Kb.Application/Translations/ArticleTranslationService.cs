    ...
    IPermissionChecker permissions, ISlugGenerator slugGenerator, TimeProvider timeProvider)
{
    ...
    public async Task<ArticleTranslationData> LinkAsync(Guid sourceId, Guid targetId, CancellationToken ct) { await RequireAsync(ct); Id(sourceId); Id(targetId); if (sourceId == targetId) throw new BusinessRuleException("An article cannot be linked to itself."); return await repository.LinkAsync(sourceId, targetId, Audit(), ct); }
